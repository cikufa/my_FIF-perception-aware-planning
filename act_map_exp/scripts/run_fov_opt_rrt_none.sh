...
manifold_bin="${FOV_MANIFOLD_BIN:-${workspace_root}/My_FoV_Optimization/Manifold_cpp/build/manifold_test_trajectory}"
...
run_along_path_opt() {
  ...
  local output_dir="$2"
  local points3d="$3"
  local along_path_input="${input_dir}/along_path/stamped_Twc.txt"
  if [[ ! -f "${along_path_input}" ]]; then
