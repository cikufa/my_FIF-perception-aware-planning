...
def main():
    ...
    with open(os.path.join(output_dir, 'cameras.txt'), 'w') as f:
        f.write(f"1 PINHOLE {args.width} {args.height} {focal} {focal} {cx} {cy}\n")

    with open(os.path.join(output_dir, 'images.txt'), 'w') as img_file:
        for idx, (xyz, pyr) in enumerate(tqdm(poses)):
            # Set camera pose (convert meters to UE cm units)
            ...
            # Convert and save pose
            pos_colmap, quat_colmap = robust_ue_to_colmap_pose(xyz, pyr)
            img_file.write(f"{idx+1} {' '.join(map(str, quat_colmap))} {' '.join(map(str, pos_colmap))} 1 {img_name}\n")
